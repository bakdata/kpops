...
def extract_config_fields_for_yaml(fields: dict[str, Any], required: bool) -> dict[str, Any]:
    extracted_fields = {}
    for key, value in fields.items():
        if issubclass(type(value), FieldInfo):
            ...
            extracted_fields[key] = extract_config_fields_for_yaml(fields[key], required)
...
def create_config(file_name: str, dir_path: Path) -> None:
    ...
    with file_path.open(mode="w") as conf:
        # Write title
        conf.write("# " + describe_object(KpopsConfig.__doc__))
        conf.write("\n" * 2)

        
        non_required = extract_config_fields_for_yaml(collect_fields(KpopsConfig), False)
