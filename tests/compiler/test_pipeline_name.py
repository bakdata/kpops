...
def test_should_set_pipeline_name_with_default_base_dir():
    ...
    assert "some-random-path-for-testing" == os.environ["pipeline_name"]
    assert "some" == os.environ["pipeline_name_0"]
    assert "random" == os.environ["pipeline_name_1"]
    assert "path" == os.environ["pipeline_name_2"]
    assert "for" == os.environ["pipeline_name_3"]
    assert "testing" == os.environ["pipeline_name_4"]
...
def test_should_set_pipeline_name_with_specific_relative_base_dir():
    ...
    assert "for-testing" == os.environ["pipeline_name"]
    assert "for" == os.environ["pipeline_name_0"]
    assert "testing" == os.environ["pipeline_name_1"]
